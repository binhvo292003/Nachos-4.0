...
int Kernel::Close(int id)
{
    ...
    if (fileSystem->openTable[id] == NULL)
    {
        return -1;
    }

    delete fileSystem->openTable[id];
    fileSystem->openTable[id] = NULL;

    return 0;
}

void Kernel::PrintNumber(int number)
{
    char buffer[10];
    sprintf(buffer, "\n%d\n", number);
...
int Kernel::Read(char *buffer, int size, int id)
{
    ...
    int result = fileSystem->openTable[id]->Read(buffer, size);
    return result;
}

int Kernel::Seek(int pos, int id)
{
    //  check file valid
    if (id < 0 || id >= 20)
    {
    ...
    if (pos == -1)
    {
        pos = fileSystem->openTable[id]->Length();
    }

    if (pos > kernel->fileSystem->openTable[id]->Length() || pos < 0) // Kiem tra lai vi tri pos co hop le khong
    {
